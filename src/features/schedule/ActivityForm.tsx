...
export function ActivityForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      plannedAmount: budgetType === "lumpsum" || budgetType === "estimated" ? Number(plannedAmount) || 0 : null,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div>
        ...
        <div className="flex gap-2 mb-3">
          {(["none", "lumpsum", "boq", "estimated"] as const).map((t) => (
            <button
              key={t}
              type="button"
            ...
            >
              {t === "none" ? "بدون" : t === "lumpsum" ? "مبلغ مقطوع" : t === "boq" ? "جدول كميات (BOQ)" : "تقديري"}
            </button>
          ))}
        </div>
        {budgetType === "lumpsum" && (
        ...
        {budgetType === "estimated" && (
          <>
            ...
            <p className="text-xs text-ink-soft mt-1.5">
              رقم مبدئي مرجعي فقط — لا يُحتسب ضمن "المخطط" بشاشة الميزانية، ولا يرتبط بأي عقد.
            </p>
          </>
        )}
        {budgetType === "boq" && (
